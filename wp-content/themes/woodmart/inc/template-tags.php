...
if( ! function_exists( 'woodmart_main_loop' ) ) {
	...
	function woodmart_main_loop() {
		...
		if ( $blog_design == 'masonry' || $blog_design == 'mask' ) {
			...
			$classes .= ' blog-spacing-' . $blog_spacing;
...
if( ! function_exists( 'woodmart_get_owl_attributes' ) ) {
	function woodmart_get_owl_attributes( $atts = array() ) {
...
if( ! function_exists( 'woodmart_header_block_cart' ) ) {
	function woodmart_header_block_cart() {
		...
		if ( ! woodmart_woocommerce_installed() || $design == 'disable' || ( ! is_user_logged_in() && woodmart_get_opt( 'login_prices' ) ) ) return;
...
if( ! function_exists( 'woodmart_header_block_search_icon' ) ) {
	function woodmart_header_block_search_icon() {
		...
		?>
			<div class="search-button <?php echo esc_attr( $classes ); ?>">
				...
				<?php if ( $header_search == 'dropdown' ): ?>
					<?php 
						woodmart_search_form( array( 'type' => 'dropdown' ) );
					?>
				<?php endif; ?>
			</div>
		<?php
...
if( ! function_exists( 'woodmart_header_block_search_form' ) ) {
	function woodmart_header_block_search_form() {
		woodmart_search_form( array(
			'show_categories' => woodmart_get_opt('search_categories')
		) );
	}
}
...
if( ! function_exists( 'woodmart_mobile_menu' ) ) {
	function woodmart_mobile_menu() {
		...
		$mobile_categories = woodmart_get_opt('mobile_categories');
		$mobile_categories_menu = woodmart_get_opt('mobile_categories_menu');
		$position = woodmart_get_opt('mobile_menu_position') ? woodmart_get_opt('mobile_menu_position') : 'left';
...
if( ! function_exists( 'woodmart_header_block_header_links' ) ) {
	function woodmart_header_block_header_links() {
		...
		if( ! empty( $links ) ) {
		?>
			<div class="woodmart-navigation woodmart-header-links">
				<ul class="menu">
					<?php foreach ($links as $key => $link):
						$classes .= ' menu-item-'. $key;
					?>
						<li class="menu-simple-dropdown item-event-hover <?php echo esc_attr( $classes ); ?>"><a href="<?php echo esc_url( $link['url'] ); ?>"><span><?php echo wp_kses( $link['label'], 'default' ); ?></span></a>
...
if( ! function_exists( 'woodmart_topbar_links' ) ) {
	...
	function woodmart_topbar_links ( $items = '', $args = array(), $return = false ) {
    	...
    	$is_topbar_menu = ! empty( $args ) && $args->theme_location == 'top-bar-menu' ;
		
		$login_side = '';

	    if ( $is_mobile_menu || $is_topbar_menu || $return ) {

	    	$is_wishlist_in_topbar = ( woodmart_get_opt( 'header_wishlist' ) == 'topbar' );
	    	$is_wishlist_in_header = ( woodmart_get_opt( 'header_wishlist' ) == 'header' );
	    	$is_account_in_topbar = woodmart_get_opt( 'topbar_links' );
	    	$is_account_in_header = woodmart_get_opt( 'header_links' );
	    	$my_account_style = woodmart_get_opt( 'header_my_account_style' );
	    	$links_with_username = woodmart_get_opt( 'links_with_username' );
	    	...
	    	if( $is_wishlist_in_topbar || ( $is_wishlist_in_header && $is_mobile_menu ) ) {
		    	// Wishlist item firstly
				$items .= '<li class="menu-item item-level-0 menu-item-wishlist">';
				$items .= woodmart_header_block_wishlist( true, $is_mobile_menu );
				$items .= '</li>';
	    	}
	    	
			$links = woodmart_get_header_links();

			if( $is_account_in_topbar || ( $is_account_in_header && $is_mobile_menu ) ){
