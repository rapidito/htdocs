...
if( ! class_exists( 'WOODMART_HB_Element' ) ) {
	abstract class WOODMART_HB_Element {
		...
		public function render( $el, $children = '' ) {
			...
			} elseif ( file_exists(TEMPLATEPATH . $path) ) {
		...
		public function generate_css( $params, $selector ) {
			...
			if( isset( $params['border'] ) ) {
				$border_css = $this->generate_border_css( $params['border'] );
			}
			...
			if( ! empty( $rules ) || ! empty( $border_css ) ) add_action( 'whb_after_header', function() use($css) { echo $css; } );
		...
		public function generate_border_css( $border ) {
			$css = '';
			...
			if( isset( $width ) ) {
				$css .= 'border-bottom-width: ' . $width . 'px;';
			}

			$css .= ( isset( $style ) ) ? 'border-bottom-style: ' . $style . ';' : 'border-bottom-style: solid;';
