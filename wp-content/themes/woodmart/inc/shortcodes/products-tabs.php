...
if( ! function_exists( 'woodmart_shortcode_products_tabs' ) ) {
	function woodmart_shortcode_products_tabs($atts = array(), $content = null) {
	    ...
	    foreach ( $tab_titles as $tab ) {
			...
			if ( empty( $tab_atts['icon_size'] ) ) $tab_atts['icon_size'] = '25x25';
		...
		ob_start(); ?>
			<div class="woodmart-products-tabs<?php echo esc_attr( $class ); ?>">
				<div class="woodmart-tabs-header">
					...
					<?php if ( ! empty( $title ) ): ?>
						<div class="tabs-name">
							<?php if ( $img_id ) echo woodmart_display_icon( $img_id, $img_size, 30 ); ?>
							<span class="tabs-text"><?php echo ($title); ?></span>
						</div>
					<?php endif; ?>
