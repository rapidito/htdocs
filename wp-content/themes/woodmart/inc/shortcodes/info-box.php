...
if( ! function_exists( 'woodmart_shortcode_info_box' ) ) {
	function woodmart_shortcode_info_box( $atts, $content ) {
		...
		$class .= ( $bg_hover_colorpicker == 'gradient' ) ? ' box-bg-gradient' : '';
		...
		ob_start(); ?>
			<div id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $class ); ?>" <?php if( ! empty( $attributes['url'] ) && empty( $btn_text ) ): ?> onclick="<?php echo esc_js( $onclick ); ?>" <?php endif; ?> >
				...
				<?php
					echo '<style>';
						if ( $bg_hover_color ) echo '#' . $id . ':hover {background-color: ' . $bg_hover_color . ' !important;}';

						if ( $bg_hover_colorpicker == 'gradient' ) echo '#' . $id . ':after {' . woodmart_get_gradient_css( $bg_hover_color_gradient ) . ' !important;}';

						if ( $icon_bg_color ) echo '#' . $id . ' .info-box-icon {background-color: ' . $icon_bg_color . ' !important;}';
						if ( $icon_bg_hover_color ) echo '#' . $id . ':hover .info-box-icon {background-color: ' . $icon_bg_hover_color . ' !important;}';


						if ( $icon_border_color ) echo '#' . $id . ' .info-box-icon {border-color: ' . $icon_border_color . ' !important;}';
						if ( $icon_border_hover_color ) echo '#' . $id . ':hover .info-box-icon {border-color: ' . $icon_border_hover_color . ' !important;}';

						if ( $icon_text_color ) echo '#' . $id . ' .with-text {color: ' . $icon_text_color . ' !important;}';
					echo '</style>';
				?>
			</div>
		<?php
...
if( ! function_exists( 'woodmart_shortcode_info_box_carousel' ) ) {
	function woodmart_shortcode_info_box_carousel( $atts = array(), $content = null ) {
		...
		ob_start(); ?>
			<div id="<?php echo esc_attr( $carousel_id ); ?>" class="info-box-carousel-wrapper info-box-spacing-<?php echo esc_attr( $slider_spacing ); ?>  woodmart-spacing-<?php echo esc_attr( $slider_spacing ); ?> info-box-per-view-<?php echo esc_attr( $slides_per_view ); ?>" <?php echo ( $owl_atts ); ?>>
