...
if( ! function_exists( 'woodmart_shortcode_products' ) ) {
	...
	function woodmart_shortcode_products( $atts, $query = false ) {
		...
		//Element title
		if ( ( ! $is_ajax || $pagination == 'arrows' ) && $element_title ) echo '<h4 class="title element-title">' . esc_html( $element_title ) . '</h4>';
			...
			?>
		    	<div class="products-footer">
		    		<?php if ( $pagination == 'more-btn' || $pagination == 'infinit' ): ?>
		    			<a href="#" rel="nofollow" class="btn woodmart-load-more woodmart-products-load-more load-on-<?php echo ( $pagination == 'more-btn' ) ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more products', 'woodmart' ); ?></span><span class="load-more-loading"><?php esc_html_e( 'Loading...', 'woodmart' ); ?></span></a>
