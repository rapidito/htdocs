...
if(function_exists('Manage_stock_from_square_function')){
	...
	curl_setopt_array($curlddd, array(
	  CURLOPT_URL => "https://connect.squareup.com/v1/".get_option('woo_square_location_id')."/inventory",
	...
	$response = curl_exec($curlddd);
	$err = curl_error($curlddd);
	curl_close($curlddd);
	Helpers::debug_log('info', "All variations ".$response );
	if (!$err and !empty($response)) {
	  $all_square_variation  = json_decode($response);
	} 
		...
		$curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, $this->square->getSquareURL() . "/items/".$item_id);
		curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE);
	...
	*/
	...
	public function update_variation($item_id,$variation_id,$POSTFIELDS){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => $this->square->getSquareURL() . "/items/".$item_id."/variations/".$variation_id,
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => "",
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 30,
